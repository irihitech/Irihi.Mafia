...
public static class DrawingContextExtensions
{
        ...
        IPen? pen)
    {
        if (element.Transform is not null)
        {
            ...
            using (context.PushTransform(transform))
            {
                context.DrawEllipse(brush, pen, new Point(element.X, element.Y), element.RadiusX,
                    element.RadiusY);
            }
        }
        ...
        {
            context.DrawEllipse(brush, pen, new Point(element.X, element.Y), element.RadiusX, element.RadiusY);
        }
    }
